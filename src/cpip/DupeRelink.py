...
"""
DupeRelink.py -- Searches for HTML files that are the same, writes a single file into a common area and deletes all the
others. Then re-links all the remaining HTML files that linked to the original files to link to the file in the common
area. This is a space saving optimisation after CPIPMain.py has processed a directory of source files.
"""
import argparse
import collections
import fnmatch
...
def _replace_in_file(fpath, text_find, text_repl, nervous_mode, len_root_dir):
    """Reads the contents of the file at fpath, replaces text_from with text_repl and
    writes it back out to the same fpath."""
    if nervous_mode:
        logging.info(
...
def _copy_delete_duplicates_fix_links(hash_result, common_dir, nervous_mode, len_root_dir):
    """Copy a single file that is duplicated to the common area, rewrite the links in that copy to the
    original location then delete all duplicates."""
    count_deleted = 0
    count_bytes_saved = 0
    ...
    for k, v in hash_result.items():
        ...
        # Need to rewrite the links in the copied file back to the directory that we copied from.
        # Change <a href="macros_noref.html#_UHlfUFlUSE9OX0hfMA__">
        # To     <a href="../includes/noddy.c/macros_noref.html#_UHlfUFlUSE9OX0hfMA__">
        text_find = LINK_FORMAT_STR.format('')
        depth_diff = 1 + copy_from.count(os.sep) - copy_to.count(os.sep)  # Add 1 as _common/ gives false extra depth
...
def _rewrite_links_where_files_deleted(root_dir, sub_dir_for_common_files, nervous_mode, hash_result, len_root_dir):
    ...
    for k, v in hash_result.items():
        for dupe_file_path in v:
            # Look at all HTML files in this directory and relink them to the common_dir/file
            for root, dirs, files in os.walk(os.path.dirname(dupe_file_path)):
                depth = root.count(os.sep)
                assert depth >= root_depth
                # Search for things like: <a href="Python.h_53f762a407a98d46db1d8ca72f382b50.html">
                # Replace with something like: <a href="../../../common/Python.h_53f762a407a98d46db1d8ca72f382b50.html">
...
def process(root_dir, sub_dir_for_common_files=SUB_DIR_FOR_COMMON_FILES, file_glob=FILE_GLOB, nervous_mode=False):
...
def main():
    ...
    program_shortdesc = 'DupeRelink.py - Delete duplicate HTML files and relink them to save space.'\
...
""" % (program_shortdesc, program_version, str(__date__))
    ...
    parser.add_argument("-s", "--subdir", type=str, dest="subdir", default=SUB_DIR_FOR_COMMON_FILES,
        help="Sub-directory for writing the common files. [default: %(default)s]")
    parser.add_argument("-n", "--nervous", action="store_true", dest="nervous", default=False,
    ...
    if os.path.isfile(inPath):
        ...
        count_deleted, count_bytes_saved = process(inPath,
                                                   sub_dir_for_common_files=args.subdir,
                                                   file_glob=FILE_GLOB,
                                                   nervous_mode=args.nervous)
        print('Files deleted: {:12d}'.format(count_deleted))
        print('  Bytes saved: {:12d} {:8.3f} (MB)'.format(count_bytes_saved, count_bytes_saved / 1024**2))
    else:
