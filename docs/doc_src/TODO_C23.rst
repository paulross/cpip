...
CPIP is based on C99 (ISO/IEC 9899:1999).
...
Or, perhaps, at a more granular level.
...
.. code-block::  python
    ...
    {
        'C23' : {
            # Caution, does True mean Trigraphs are supported or the C23
            # standard is supported since Trigraphs are illegal in C23?
...
The option``--cfg ?`` would show all defaults and exit.
...
C17     ``__STDC_VERSION__ = 201710L``      ISO/IEC 9899:2018
...
.. code-block:: python
    ...
    class CStandard:
        ...
        C_STANDARDS_SUPPORTED = ['k&r', 'ansi', ] # And so on.
        STDC_VERSION = {
            'c11' : '201112L',
            # Etc.
        }
...
* `C17 <https://en.wikipedia.org/wiki/C17_(C_standard_revision)>`_
